<div class="space-y-3 p-3">
    ...
    <table
        class="table table-sm table-pin-rows table-pin-cols border border-gray-200 rounded-none"
    >
        ...
        <tr>
            ...
            <td style="font-size: 11px">
                @switch($task->status) @case('pending')
                <span
                    class="px-4 bg-yellow-200 text-yellow-800 rounded-xl"
                    >{{ $task->status }}</span
                >
                ...
                @break @case('assigned')
                <span
                    class="px-4 bg-blue-200 text-blue-800 rounded-xl"
                    >{{ $task->status }}</span
                >
                @break @case('confirmed')
                <span
                    class="px-4 bg-green-200 text-green-800 rounded-xl"
                    >{{ $task->status }}</span
                >
                @break @case('closed')
                <span
            ...
            <td>
                ...
                <span style="font-size: 11px"
                    >@tanggal($task->tanggal) @jam($task->jam)</span
                >
                @endif
            </td>
            <td class="flex gap-1 items-center w-fit">
                <div>
                    <label for="taskEdit_{{$task->id}}" class="cursor-pointer">
                        <span
                            class="material-symbols-rounded text-slate-800"
                ...
                <div>
                    <label for="taskView_{{$task->id}}" class="cursor-pointer">
                        <span
                            class="material-symbols-rounded text-blue-800"
                            style="font-size: 18px"
                            >find_in_page</span
                        >
                    </label>
                ...
                <a href="{{route('tasks.delete', ['id' => $task->id])}}">
                    <span class="material-symbols-rounded text-red-800"
                        >delete</span
                    >
                </a>
            </td>
